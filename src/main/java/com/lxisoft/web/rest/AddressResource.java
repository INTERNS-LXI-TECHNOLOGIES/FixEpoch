...
/**
 * REST controller for managing {@link com.lxisoft.domain.Address}.
 */
@RestController
@RequestMapping("/api")
public class AddressResource {
    ...
    public ResponseEntity<List<AddressDTO>> getAllAddresses(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("userextra-is-null".equals(filter)) {
            log.debug("REST request to get all Addresss where userExtra is null");
            return new ResponseEntity<>(addressService.findAllWhereUserExtraIsNull(),
                    HttpStatus.OK);
        }
